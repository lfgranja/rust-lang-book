...
- **`src/`**: The book content (Markdown). This is the primary workspace.
- **`packages/trpl/`**: The `trpl` crate, used in book examples. It provides a stable API for readers.
- **`packages/tools/`**: Internal maintenance scripts (Rust binaries).
...
3.  **Validation Loop**:
    - Make changes.
    - Run `dprint fmt` / `rustfmt`.
    - Run `mdbook test` on the specific chapter.
    - Run `ci/validate.sh`.
4.  **No Starch Constraints**: Never touch `nostarch/`. If you need to verify print output, use `packages/tools` scripts, but generally rely on `src/` validation.
